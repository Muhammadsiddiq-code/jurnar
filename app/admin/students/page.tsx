      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="flex items-start sm:items-center justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted text-xs sm:text-sm lg:text-base">Manage student information</p>
          </div>
          <button
            onClick={() => setShowForm(true)}
            className="p-2.5 sm:p-3 rounded-lg sm:rounded-xl bg-gradient-to-r from-primary to-secondary text-white shadow-lg hover:opacity-90 transition-all flex-shrink-0"
          >
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search students..."
            className="w-full px-4 py-2.5 sm:py-3 pl-10 sm:pl-12 rounded-lg sm:rounded-xl border border-input bg-card text-foreground text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-primary transition-all"
          />
          <svg
            className="w-4 h-4 sm:w-5 sm:h-5 text-muted absolute left-3 sm:left-4 top-1/2 -translate-y-1/2"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
